...
import { playSuccessSound, playCountdownBeep } from '../services/audioUtils';
...
const Player: React.FC<Props> = ({ plan, onExit }) => {
  ...
  useEffect(() => {
    ...
    if (match) {
      actionName = match[1].toUpperCase().trim();
    } else {
      // 如果没有括号，尝试直接匹配常见动作名称
      const upperName = exerciseName.toUpperCase();
      if (upperName.includes('跳') || upperName.includes('JUMP')) {
        actionName = 'JUMP';
      } else if (upperName.includes('蹲') || upperName.includes('SQUAT')) {
        actionName = 'SQUAT';
      } else if (upperName.includes('开合') || upperName.includes('JACK')) {
        actionName = 'JUMPING JACK';
      } else {
        // 默认尝试使用整个名称
        actionName = upperName;
      }
    }

    // 尝试获取动作，如果没有找到且包含空格，尝试取第一个单词（兼容性）
    let action = getAction(actionName);
    if (!action && actionName.includes(' ')) {
      action = getAction(actionName.split(' ')[0]);
    }
  ...
  useEffect(() => {
    ...
    const prepareExercise = async () => {
      ...
      setActionAccuracy(0);
      instructionSuccessCount.current = 0; // 重置连续成功计数
  ...
  useEffect(() => {
    if ((state === PlayerState.COUNTDOWN || state === PlayerState.PLAYING || state === PlayerState.INSTRUCTION) && videoRef.current && streamRef.current) {
      ...
      const detectionCallback = (result: any) => {
        ...
        requestAnimationFrame(() => {
          ...
          if (result.landmarks && result.landmarks.length > 0) {
            ...
            // 使用动作组件的检测器
            if (currentActionRef.current && (state === PlayerState.PLAYING || state === PlayerState.INSTRUCTION)) {
              ...
              if (detectionResult.isCompleted) {
                if (state === PlayerState.PLAYING) {
                  ...
                  // 引导模式下，连续成功2次自动开始
                  instructionSuccessCount.current += 1;
                  if (audioContextRef.current) playSuccessSound(audioContextRef.current);

                  if (instructionSuccessCount.current >= 2) {
  ...
  useEffect(() => {
    if (state === PlayerState.PLAYING || state === PlayerState.INSTRUCTION) {
      ...
      const patternSteps = beatPattern.pattern;
  ...
  if (state === PlayerState.INSTRUCTION || state === PlayerState.PREPARING) {
    return (
      <div className="fixed inset-0 z-50 bg-slate-900 flex flex-col items-center justify-between p-8 text-center animate-fade-in overflow-hidden">
        {/* Top Header */}
        <div className="w-full pt-8">
          <p className="text-yellow-400 font-bold tracking-widest uppercase text-sm animate-pulse mb-2">NEXT UP</p>
        ...
        <div className="flex-1 w-full flex items-center justify-center my-6">
          <div className="relative w-full max-w-sm aspect-square bg-slate-800/50 backdrop-blur-md rounded-[2.5rem] overflow-hidden border border-white/10 shadow-[0_0_60px_rgba(45,212,191,0.15)] flex items-center justify-center group">
            <div className="absolute inset-0 bg-gradient-to-br from-teal-500/5 via-transparent to-blue-500/5 opacity-50 group-hover:opacity-100 transition-opacity duration-700"></div>
        ...
        <div className="w-full pb-10 space-y-6">
          <div className="flex items-center justify-center">
            {cameraReady ? (
              <div className="flex items-center gap-2 text-green-400 bg-green-500/10 px-4 py-2 rounded-full border border-green-500/20 backdrop-blur-sm">
                <Camera className="w-3.5 h-3.5" />
                <span className="text-[10px] font-bold tracking-widest">CAMERA READY</span>
              </div>
            ) : (
              <div className="flex items-center gap-2 text-red-300 bg-red-500/10 px-4 py-2 rounded-full border border-red-500/20 backdrop-blur-sm">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-30 px-6 pb-10 flex flex-col gap-2 bg-gradient-to-t from-black via-black/80 to-transparent pt-16">
        <div className="flex justify-center items-end gap-2 mb-2">
          {['DONG', 'CI', 'DA', 'CI'].map((label, idx) => (
            <div
              key={idx}
              className={`h-8 flex items-center justify-center rounded-xl transition-all duration-150 ${beatStep === idx
