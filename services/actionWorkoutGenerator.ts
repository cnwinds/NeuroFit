...
import { getAllActions, selectRandomActions, getActionsByCategory, ActionComponent, getAction } from '../actions';

/**
 * 根据训练重点获取对应的动作类别和部位
 */
function getFiltersFromFocus(focus: string): { categories: string[], parts: string[] } {
  const focusLower = focus.toLowerCase();

  if (focusLower.includes('核心') || focusLower.includes('腹肌')) {
    return { categories: ['strength', 'flexibility'], parts: ['core'] };
  }
  if (focusLower.includes('上肢')) {
    return { categories: ['strength', 'cardio'], parts: ['arms', 'shoulders', 'chest', 'back'] };
  }
  if (focusLower.includes('下肢') || focusLower.includes('爆发')) {
    return { categories: ['strength', 'cardio'], parts: ['legs'] };
  }
  if (focusLower.includes('灵活') || focusLower.includes('体态')) {
    return { categories: ['flexibility', 'balance'], parts: ['full-body', 'core', 'neck'] };
  }
  if (focusLower.includes('有氧') || focusLower.includes('hiit')) {
    return { categories: ['cardio'], parts: ['full-body', 'legs', 'arms'] };
  }

  // 默认：全身训练
  return {
    categories: ['cardio', 'strength', 'flexibility', 'balance'],
    parts: ['full-body', 'arms', 'legs', 'core']
  };
}
...
function actionToExercise(action: ActionComponent, difficulty: Difficulty): Exercise {
  ...
  const partsText = action.targetParts.map(p => {
    const map: Record<string, string> = {
      'arms': '手臂', 'legs': '下肢', 'core': '核心', 'full-body': '全身',
      'shoulders': '肩部', 'back': '背部', 'chest': '胸部', 'neck': '颈部'
    };
    return map[p] || p;
  }).join('、');
  ...
  return {
    ...
    description: `${action.name}动作，重点锻炼${partsText}。${descriptions[action.category] || '全面激活身体机能。'}`,
...
export function generateWorkoutPlanFromActions(
  ...
  // 根据训练重点获取过滤器
  const filters = getFiltersFromFocus(focusArea);

  // 筛选动作
  let selectedActions = allActions.filter(action => {
    // 匹配类别
    const categoryMatch = filters.categories.includes(action.category);
    // 匹配部位（只要有一个部位重合即可）
    const partMatch = action.targetParts.some(part => filters.parts.includes(part));

    return categoryMatch || partMatch;
  });

  // 如果没有找到特定匹配的动作，退而求其次选择类别匹配的
  if (selectedActions.length === 0) {
    selectedActions = allActions.filter(action => filters.categories.includes(action.category));
  }
  ...
  // 强制第一个动作为 "SAY HI"
  const sayHiAction = getAction('SAY HI');
  let finalActions: ActionComponent[] = [];

  // 剩余需要生成的动作数量 (至少为 0)
  const remainingCount = Math.max(0, estimatedActionCount - (sayHiAction ? 1 : 0));

  if (remainingCount > 0) {
    if (remainingCount <= uniqueActions.length) {
      // 随机选择指定数量的动作
      finalActions = selectRandomActions(remainingCount);
    } else {
      // 如果需要的数量超过可用数量，先使用所有动作，然后随机补充
      finalActions = [...uniqueActions];
      const remainingToAdd = remainingCount - uniqueActions.length;
  ...
  // 将 SAY HI 添加到队首
  if (sayHiAction) {
    finalActions.unshift(sayHiAction);
  }
